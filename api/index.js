...
// Simple email configuration test endpoint (admin only)
app.post('/api/test-email-config', async (req, res) => {
  ...
  try {
    ...
    console.log(`Admin ${user.username} testing email configuration`);
    ...
    const emailConfig = {
      ...
      emailSecure: process.env.EMAIL_SECURE || 'not set',
      emailFrom: process.env.EMAIL_FROM || 'not set',
      maskedEmailUser: process.env.EMAIL_USER ? process.env.EMAIL_USER.replace(/(.{3}).*@/, '$1***@') : 'not set'
    };
    ...
    try {
      ...
      return res.status(200).json({
        ...
        config: emailConfig,
        emailFunctionsImported: true,
        status: emailConfig.hasEmailUser && emailConfig.hasEmailPass ? 'OK' : 'MISSING_CREDENTIALS'
      });
    } catch (importError) {
      return res.status(500).json({
        message: 'Email configuration test failed',
        config: emailConfig,
        emailFunctionsImported: false,
