  ...
  */
  ...
  app.post("/api/manager/stripe-connect/create", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      ...
      const { createConnectAccount } = await import('./services/stripe-connect-service');
  ...
  app.get("/api/manager/stripe-connect/onboarding-link", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      const managerId = req.neonUser!.id;
  ...
  app.get("/api/manager/stripe-connect/status", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      const managerId = req.neonUser!.id;
  ...
  app.post("/api/manager/revenue/sync-payments", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      const managerId = req.neonUser!.id;
  ...
  app.get("/api/manager/payments/booking/:bookingId/status", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      ...
      const { getPaymentStatus } = await import('./services/payment-tracking-service');
  ...
  app.post("/api/manager/payments/booking/:bookingId/sync", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      const bookingId = parseInt(req.params.bookingId);
      ...
      const { syncPaymentStatusFromStripe } = await import('./services/payment-tracking-service');
  ...
  app.post("/api/manager/payments/booking/:bookingId/recover", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      const bookingId = parseInt(req.params.bookingId);
      ...
      const { recoverMissingPaymentData } = await import('./services/payment-tracking-service');
  ...
  app.post("/api/manager/kitchens", requireFirebaseAuthWithUser, requireManager, async (req: Request, res: Response) => {
    try {
      // Firebase auth verified by middleware - req.neonUser is guaranteed to be a manager
      const user = req.neonUser!;
      ...
      // Verify the manager has access to this location
      const locations = await firebaseStorage.getLocationsByManager(user.id);
      const hasAccess = locations.some(loc => loc.id === locationIdNum);
      if (!hasAccess) {
        return res.status(403).json({ error: "Access denied to this location" });
      }
  ...
  app.get("/api/chef/kitchens/:kitchenId/pricing", requireChef, async (req: Request, res: Response) => {
    try {
      const kitchenId = parseInt(req.params.kitchenId);
      if (isNaN(kitchenId) || kitchenId <= 0) {
        return res.status(400).json({ error: "Invalid kitchen ID" });
  ...
  app.post("/api/bookings/checkout", async (req: Request, res: Response) => {
    try {
