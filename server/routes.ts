...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  */
  ...
  // ===== MANAGER MANUAL BOOKINGS (BLOCK TIME) =====
  
  // Create a manual booking (block time)
  app.post("/api/manager/kitchens/:kitchenId/manual-bookings", async (req: Request, res: Response) => {
    try {
      ...
      const kitchenId = parseInt(req.params.kitchenId);
      const { bookingDate, startTime, endTime, specialNotes, bookingType } = req.body;

      // Validate booking type
      const validTypes = ["manager_blocked", "external"];
      const type = bookingType || "manager_blocked";
      
      if (!validTypes.includes(type)) {
        return res.status(400).json({ error: "Invalid booking type. Must be 'manager_blocked' or 'external'" });
      }
      ...
      const booking = await firebaseStorage.createKitchenBooking({
        chefId: null as any, // No chef for manual bookings
        kitchenId,
      ...
      console.log(`✅ Manager ${user.id} created manual booking for kitchen ${kitchenId}`);
      res.json(booking);
    } catch (error: any) {
  ...
  // Get available slots for a date (for manager UI)
  app.get("/api/manager/kitchens/:kitchenId/available-slots", async (req: Request, res: Response) => {
    try {
      // Check authentication
      const sessionUser = await getAuthenticatedUser(req);
      const isFirebaseAuth = req.neonUser;
  ...
  app.delete("/api/manager/manual-bookings/:bookingId", async (req: Request, res: Response) => {
    try {
      // Check authentication
      const sessionUser = await getAuthenticatedUser(req);
      const isFirebaseAuth = req.neonUser;
      ...
      // Get booking to verify it's a manual booking and belongs to this manager
      const booking = await firebaseStorage.getBookingById(bookingId);
      
      if (!booking) {
        return res.status(404).json({ error: "Booking not found" });
      }

      if (booking.bookingType !== "manager_blocked" && booking.bookingType !== "external") {
        return res.status(403).json({ error: "Can only delete manual bookings" });
      }

      // Delete the booking
      await firebaseStorage.deleteKitchenBooking(bookingId);
      
      console.log(`✅ Manager ${user.id} deleted manual booking ${bookingId}`);
      res.json({ success: true });
    } catch (error: any) {
      console.error("Error deleting manual booking:", error);
      res.status(500).json({ error: error.message || "Failed to delete manual booking" });
    }
  });
