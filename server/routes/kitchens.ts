    ...
    try {
        ...
        if (phone !== undefined) {
            if (phone && phone.trim() !== "") {
                ...
                // A phone is a login identifier, so it resolves to exactly one
                // account. Firebase only blocks a number that is ALREADY a credential
                // on another user, so a number that merely sits on somebody's profile
                // passes the check above — and would then fail the unique index as an
                // opaque 500, after an SMS had been sent and the number linked to THIS
                // account. Ask the database first and answer cleanly.
