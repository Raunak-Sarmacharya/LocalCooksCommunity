  ...
  return (
    <section id="about" className="py-10 sm:py-12 md:py-16 px-4 sm:px-6 scroll-mt-24 bg-gradient-to-b from-white to-gray-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5 pointer-events-none">
        <div className="absolute top-20 right-20 w-64 sm:w-80 md:w-96 h-64 sm:h-80 md:h-96 bg-brand-primary rounded-full blur-3xl"></div>
        <div className="absolute bottom-20 left-20 w-56 sm:w-64 md:w-80 h-56 sm:h-64 md:h-80 bg-gold rounded-full blur-3xl"></div>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 sm:gap-8 md:gap-12 items-center mb-8 sm:mb-10 md:mb-12">
          ...
          >
            <div className="relative">
              <div className="absolute -top-6 -left-6 w-32 h-32 bg-brand-primary/10 rounded-full blur-2xl"></div>
              <div className="absolute -bottom-6 -right-6 w-40 h-40 bg-brand-primary/5 rounded-full blur-2xl"></div>
            ...
            {values.map((value, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg sm:text-xl md:text-2xl font-bold mb-1.5 sm:mb-2 md:mb-3 text-brand-text group-hover:text-brand-primary transition-colors duration-300">{value.title}</h3>
                  <p className="text-brand-text font-sans text-sm sm:text-base md:text-lg leading-relaxed">{value.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
