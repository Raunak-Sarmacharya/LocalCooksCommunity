  ...
  return (
    <section id="how-it-works" className="py-10 sm:py-12 md:py-16 px-4 sm:px-6 scroll-mt-24 bg-gradient-to-b from-white via-gray-50 to-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5 pointer-events-none">
        <div className="absolute top-40 left-10 w-64 sm:w-80 md:w-96 h-64 sm:h-80 md:h-96 bg-[var(--color-primary)] rounded-full blur-3xl"></div>
        <div className="absolute bottom-40 right-10 w-56 sm:w-64 md:w-80 h-56 sm:h-64 md:h-80 bg-[var(--color-gold)] rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="p-6 sm:p-8 md:p-10 relative z-10">
                <h3 className="text-xl sm:text-2xl md:text-3xl font-bold mb-3 sm:mb-4 text-[var(--color-text-primary)] group-hover:text-[var(--color-primary)] transition-colors duration-300">{step.title}</h3>
                <p className="text-[var(--color-text-primary)] leading-relaxed font-sans text-sm sm:text-base md:text-lg">{step.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 md:p-10 lg:p-14 border border-gray-100 relative overflow-hidden group">
          ...
          <div className="grid md:grid-cols-2 gap-8 sm:gap-10 md:gap-12 items-center relative z-10">
            <div>
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-5 text-[var(--color-text-primary)] group-hover:text-[var(--color-primary)] transition-colors duration-300">Everything You Need to Succeed</h3>
              <p className="text-base sm:text-lg md:text-xl text-[var(--color-text-primary)] mb-6 sm:mb-8 md:mb-10 leading-relaxed font-sans">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
                {benefits.map((benefit, index) => (
                  ...
                  >
                    <div className="flex-shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-gradient-to-br from-[#F51042]/15 to-[#F51042]/5 flex items-center justify-center text-[#F51042] group-hover/item:bg-gradient-to-br group-hover/item:from-[#F51042]/25 group-hover/item:to-[#F51042]/10 group-hover/item:scale-110 transition-all duration-300 shadow-md">
                      {benefit.icon}
                    </div>
                    <div>
                      <h4 className="font-bold text-base sm:text-lg text-[var(--color-text-primary)] mb-1.5 sm:mb-2 group-hover/item:text-[var(--color-primary)] transition-colors duration-300">{benefit.title}</h4>
                      <p className="text-[var(--color-text-primary)] leading-relaxed font-sans text-sm sm:text-base">{benefit.description}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            >
              <div className="bg-gradient-to-br from-[var(--color-primary)]/15 via-[var(--color-primary)]/10 to-[var(--color-cream)] rounded-xl sm:rounded-2xl shadow-xl p-8 sm:p-10 md:p-12 lg:p-16 h-full flex items-center justify-center relative overflow-hidden">
                {/* Animated background pattern */}
                <div className="absolute inset-0 opacity-10">
                  <div className="absolute top-0 right-0 w-32 sm:w-40 h-32 sm:h-40 bg-[var(--color-primary)] rounded-full blur-3xl"></div>
                  <div className="absolute bottom-0 left-0 w-24 sm:w-32 h-24 sm:h-32 bg-[var(--color-gold)] rounded-full blur-3xl"></div>
                </div>
                ...
                <div className="text-center relative z-10">
                  <div className="mb-4 sm:mb-6 inline-block p-3 sm:p-4 bg-white/20 rounded-xl sm:rounded-2xl backdrop-blur-sm group-hover/feature:scale-110 transition-transform duration-300">
                    <Utensils className="h-16 w-16 sm:h-20 sm:w-20 text-[var(--color-primary)] mx-auto" />
                  </div>
                  <p className="text-lg sm:text-xl md:text-2xl font-bold text-[var(--color-text-primary)] font-sans leading-relaxed px-2">Focus on cooking while we handle the business side</p>
                </div>
              </div>
            </motion.div>
          </div>
