...
>(({ className, children, showCloseButton = true, overlayClassName, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      // every dialog in this app: all of them are modal (the only `modal={false}`
      // usages are DropdownMenus, not dialogs), so the content outside really is
      // obscured and non-interactive. `{...props}` still spreads afterwards, so a
      // genuinely non-modal dialog can opt out by passing `aria-modal={undefined}`.
      aria-modal="true"
      className={cn(
    ...
    >
      {children}
      {showCloseButton ? (
        <DialogPrimitive.Close className="absolute right-3 top-3 w-8 h-8 flex items-center justify-center z-50 focus:outline-none hover:bg-muted rounded-full transition-colors duration-200">
